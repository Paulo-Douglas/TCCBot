    ...
    <div className="border border-[#364153] rounded-lg overflow-hidden bg-[#1E2939] hover:border-[#4B5563] transition-colors">
      ...
      >
        <div className="flex items-center gap-3 flex-1 text-left">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-medium text-white break-words">{tcc.title}</h3>
            <p className="text-xs text-gray-400 mt-0.5 truncate">{tcc.author}</p>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="px-4 pb-4 pt-2 border-t border-[#364153] space-y-3">
          <InfoRow icon={User} label="Autor(a)" value={tcc.author} />
          {tcc.advisor && <InfoRow icon={User} label="Orientador(a)" value={tcc.advisor} />}
          <InfoRow icon={BookOpen} label="Curso" value={tcc.course} />
          ...
          {tcc.abstract && (
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-300 line-clamp-3">{tcc.abstract}</p>
              </div>
            </div>
          )}
          ...
          {tcc.score && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400">{(tcc.score * 100).toFixed(0)}%</span>
            </div>
          )}
